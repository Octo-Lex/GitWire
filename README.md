...
## Policy Preview & Simulation

GitWire helps operators understand policy changes before they affect repositories.

The policy preview workflow is intentionally **non-mutating**: it does not save config, enqueue jobs, or write to GitHub.

Operators can:

* **Validate** `.gitwire.yml` and inspect structured errors, warnings, enabled pillars, dry-run state, risky settings, and normalized redacted config.
* **Preview** policy behavior in the dashboard before rollout.
* **Simulate** proposed policy against historical GitWire decisions to see what would act, skip, dry-run, block, or require AI replay.
* **Compare impact** between the current repo policy and a proposed policy, including dry-run changes, pillar enablement, risk changes, warning changes, and per-event outcome changes.
* **Review guardrail recommendations** generated by deterministic rules for safer rollout.

### Recommended rollout workflow

```mermaid
graph LR
    V[Validate] --> P[Preview]
    P --> S[Simulate]
    S --> C[Compare Impact]
    C --> R[Review Recommendations]
    R --> E[Enable with dry-run]
```

1. **Validate** the proposed policy.
2. **Confirm** dry-run and enabled pillars.
3. **Simulate** against recent repository history.
4. **Compare** proposed behavior against current behavior.
5. **Review** guardrail recommendations.
6. **Roll out** with dry-run first when enabling risky or newly permissive automation.

### Limitations

* AI-dependent outcomes (triage, AI review, issue-fix) are marked `would_require_ai` — simulation does not fabricate model outputs.
* Simulation is policy/guard-focused: it evaluates pillar enablement, trigger filters, and dry-run status, not full AI replay.
* Historical events are drawn from the `decision_log` within the selected time window.

### API endpoints

| Endpoint | Purpose |
|---|---|
| `POST /api/config/validate` | Validate policy YAML, return errors/warnings/risks |
| `POST /api/config/simulate` | Replay proposed policy against historical events |
| `POST /api/config/diff-impact` | Compare current vs proposed policy behavior |
| `POST /api/config/recommendations` | Generate deterministic guardrail recommendations |

See [Policy Preview API](docs/api/policy-preview.md) for full reference.
