...
DECLARE
  ...
  v_decision_principal   uuid;
  v_decision_action      text;
  v_decision_resource    text;
  v_decision_target      text;
  v_decision_installation bigint;
  v_decision_repository   bigint;
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    SELECT decision, principal_id, action, target_resource_type,
           target_resource_id, target_installation_id, target_repository_id
      INTO v_decision, v_decision_principal, v_decision_action,
           v_decision_resource, v_decision_target,
           v_decision_installation, v_decision_repository
      FROM auth_authorization_decisions
    ...
    -- Decision principal must be the initiating principal
...
BEGIN
  ...
  -- CAS: the attempt_id is the authentication proof. It was generated
  -- by acquire_delegation_claim (a SECURITY DEFINER function that set
  -- it via an atomic UPDATE) and is known only to the worker process
  -- that made that call. p_worker_principal_id is a supplementary
  -- defense-in-depth check; the attempt_id is the binding proof.
  UPDATE auth_delegations
...
DECLARE
  ...
  v_db_session         text := session_user;
BEGIN
  -- Validate the denial decision: must be an allow for auth_delegation:manage
  -- targeting the EXACT delegation being denied.
  SELECT principal_id, target_resource_id
    INTO v_decision_principal, v_decision_target
    FROM auth_authorization_decisions
    WHERE id = p_authorization_decision_id
      AND decision = 'allow'
      AND action = 'manage'
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'deny_delegation: authorization_decision_id % is not a valid auth_delegation:manage allow',
      p_authorization_decision_id;
  END IF;

  -- Decision target must match the exact delegation
  IF v_decision_target IS NULL OR v_decision_target != p_delegation_id::text THEN
...
-- auth_credentials: app can UPDATE only revocation fields + updated_at
GRANT SELECT, INSERT ON auth_credentials TO gitwire_app;
GRANT UPDATE (revoked_at, revoked_by, revocation_reason, updated_at) ON auth_credentials TO gitwire_app;

-- auth_principal_roles: app can UPDATE only revocation fields
...
-- auth_break_glass_activations: app can UPDATE alert + revocation fields
GRANT SELECT, INSERT ON auth_break_glass_activations TO gitwire_app;
GRANT UPDATE (alert_sent, alert_acknowledged, alert_acknowledged_at,
...
`auth_role_retirement_log`.

**Also created in this stage (after decisions table exists):**
- `auth_role_retirement_log` table + append-only triggers
- `retire_role(uuid, uuid)` SECURITY DEFINER function
- `GRANT SELECT, UPDATE ON auth_roles TO gitwire_auth_fn_owner`
- `GRANT SELECT ON auth_authorization_decisions TO gitwire_auth_fn_owner`
- `GRANT INSERT ON auth_role_retirement_log TO gitwire_auth_fn_owner`
- Column-level GRANT statements (§19 concrete GRANT SQL)
- `enforce_break_glass_decision_at_creation()` BEFORE INSERT trigger on decisions
...
**Rollback:** drop triggers, functions, then drop FKs and tables in reverse
dependency order:
1. Drop deferred FK: `ALTER TABLE auth_authorization_decisions DROP CONSTRAINT fk_decision_break_glass;`
2. Drop `retire_role(uuid, uuid)` function
3. Drop retirement log append-only triggers + function, then `DROP TABLE auth_role_retirement_log`
4. Drop break-glass activation constraint trigger, then `DROP TABLE auth_break_glass_activations`
5. Drop break-glass decision INSERT trigger
6. `DROP TABLE auth_sessions`
7. Drop decision triggers (append-only, action-valid, break-glass INSERT), then `DROP TABLE auth_authorization_decisions`
8. `DROP TABLE auth_worker_ceiling_permissions`
9. Drop ceiling triggers, then `DROP TABLE auth_worker_ceilings`
...
-- 8. CREATE FUNCTION enforce_no_subtype_retype_with_deps() + trg_no_subtype_retype
--    (moved from stage 039; references auth_worker_ceilings + auth_delegations)
-- 9. CREATE CONSTRAINT TRIGGER trg_denied_terminal
-- 10. CREATE CONSTRAINT TRIGGER trg_no_direct_executing_transition
-- 11. CREATE CONSTRAINT TRIGGER trg_delegation_worker_service
-- 12. CREATE TRIGGER trg_delegation_decision_notnull_insert
-- 13. GRANT DELETE ON auth_authorization_decisions, auth_reconciliation_log TO gitwire_auth_fn_owner
-- 14. GRANT INSERT ON auth_reconciliation_log TO gitwire_auth_fn_owner
```

**Rollback:** reverse dependency order:
1. Drop constraint triggers: `DROP TRIGGER trg_denied_terminal ON auth_delegations;`
   `DROP TRIGGER trg_no_direct_executing_transition ON auth_delegations;`
   `DROP TRIGGER trg_delegation_worker_service ON auth_delegations;`
...
2. Drop functions (exact signatures):
   ...
   `DROP FUNCTION enforce_no_subtype_retype_with_deps();`
   `DROP FUNCTION enforce_delegation_decision_at_creation();`
   `DROP FUNCTION enforce_denied_terminal();`
   `DROP FUNCTION enforce_no_direct_executing_transition();`
   `DROP FUNCTION enforce_delegation_worker_service();`
3. Drop reconciliation log append-only triggers + function, then
   `DROP TABLE auth_reconciliation_log;`
4. Drop the reverse FKs added to `auth_authorization_decisions`:
   ```sql
   ...
   ```
5. Drop the FK from `auth_delegations` to decisions:
   ```sql
