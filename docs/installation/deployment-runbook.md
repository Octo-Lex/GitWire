...
## Immutable (pull-based) Deployment

> **v0.23.0+ — the production deployment model.** This section supersedes the
> manual rebuild checklist below for routine releases. The manual checklist
> remains as a fallback and for disaster recovery.

### How it works

Production **never builds images**. The three release services
(`gitwire-app`, `gitwire-executor-service`, `dashboard`) consume
**digest-pinned** images published to GHCR by the CI `publish-release-images`
job. The four secondary services (bot, landing, docs, demo) use **persistent
local image references** and are neither pulled nor recreated during a release.

`docker-compose.yml` is the **production base**: every application service
references an image via a mandatory `${VAR:?}` variable (Compose fails if any
is unset). `docker-compose.build.yml` is the **dev/CI override** that adds
`build:` blocks for local development.

### Automated path (workflow_run)

When CI completes successfully on `master`, the `Deploy to Production` workflow
runs automatically. It:

1. Verifies the triggering run was a successful, same-repository, master push.
2. Checks out the **exact tested SHA** (`workflow_run.head_sha`) — not
   `origin/master`.
3. Downloads the triggering run's `production-release-manifest` artifact and
   validates the SHA + workflow-run ID binding.
4. Pulls the three release images **by digest** and recreates them with
   `--no-build --no-deps` in staged order (executor → app → dashboard), gating
   each stage.
5. Verifies running container image IDs match the manifest.
6. Asserts all seven non-release services are unchanged.
7. Atomically persists `releases/<sha>/images.env` and advances the
   `releases/current` symlink **only after** every gate passes.

`scripts/deploy-release.sh` is **workflow-operated only**. There is no implicit
manual fallback. A future `--manual` mode (with its own manifest verification)
is tracked separately.

### One-time transition (before the first immutable deploy)

Before the first immutable deployment, an operator must run **once**:
...
This tags the four secondary services' currently-running images with persistent
local references (`gitwire-local/<svc>:pre-immutable`) and writes the marker
`/opt/gitwire/releases/.immutable-transition-ready`. The deployment preflight
requires this marker. The tool is independent of the new Compose interpolation
(it discovers containers via Docker labels, not by parsing compose yaml).

### Production environment files

Production uses **two** env files, consumed by Compose in a fixed order:

**`/opt/gitwire/config/production.env`** (stable, mode `0600`, operator-managed):
```dotenv
DB_PASSWORD=<real database password>
GITWIRE_EXECUTOR_SERVICE_TOKEN=<shared app/executor token>
...
GITWIRE_VALIDATOR_IMAGE_REF=<validator-repo>@sha256:<64-hex-digest>
GITWIRE_VALIDATOR_IMAGE_DIGEST=sha256:<64-hex-digest>
```

**`/opt/gitwire/releases/<sha>/images.env`** (per-release, generated by
`deploy-release.sh` from the validated manifest):
```dotenv
GITWIRE_APP_IMAGE=ghcr.io/octo-lex/gitwire-app@sha256:...
GITWIRE_EXECUTOR_IMAGE=ghcr.io/octo-lex/gitwire-executor-service@sha256:...
GITWIRE_DASHBOARD_IMAGE=ghcr.io/octo-lex/gitwire-dashboard@sha256:...
```

> **Distinction:** these root-level env files are Compose **interpolation**
> inputs (read at `docker compose` time). `packages/web/.env` is the app
> container's `env_file` (read at container runtime). They are separate files
> with separate purposes.

### Deployment vs. current (staged env)

During deployment, the script uses a **staged release env** written from the
manifest — NOT `releases/current/images.env`, which may not exist yet (first
deploy) or may point to the previous release. `releases/current` advances only
after all gates pass. Post-deploy operator commands use `current`:

```bash
# Operator commands AFTER a successful deploy (uses releases/current):
docker compose -p gitwire --project-directory /opt/gitwire \
  -f /opt/gitwire/docker-compose.yml \
  --env-file /opt/gitwire/config/production.env \
  --env-file /opt/gitwire/releases/current/images.env \
  ps   # or logs, etc.
```

### Validator image

The validator image is an **independently published, immutable production
dependency**. It is NOT published by the app/executor/dashboard release
pipeline. Production must configure its identity (ref + digest) in
`production.env`. The deployment hard-gates on executor `/health.ready = true`,
which requires both Docker socket reachability AND complete validator identity.

### Gates (all fail-closed)

- **Preflight**: transition marker, secondary refs resolve, `production.env`
  mode 0600, validator identity complete, project name `gitwire`, Postgres +
  Redis healthy.
