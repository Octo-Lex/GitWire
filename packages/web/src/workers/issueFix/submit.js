  ...
  try {
    ...
    if (!(await checkAndMark("issue_fix", idempotencyKey))) {
      ...
      await postIssueComment(octokit, owner, repoName, issueNumber,
        "ℹ️ **GitWire Fix - retry deferred**\n\n" +
        "A repository-scoped submission marker already exists for this issue, so GitWire did not mutate GitHub. " +
        "This can mean an earlier attempt is still active or its external outcome is not yet safe to repeat.\n\n" +
        "Inspect `" + branchName + "` and any matching pull request before retrying. " +
        "The legacy marker expires automatically; `/gitwire run fix` is the explicit maintainer retry control after the prior outcome has been verified."
      );
      return;
    }
