...
function deriveTarget(eventName, payload) {
  ...
  return { type: prBacked ? "pr" : "issue", number: payload.pull_request?.number || payload.issue?.number || null, prBacked };
}

function validateAction(action, target) {
  const name = action?.action;
  const args = action?.args || {};
  if (!SUPPORTED_ACTIONS.has(name)) return `Unknown custom rule action: ${name || "<missing>"}`;
...
async function fetchFreshPRContext(octokit, owner, repo, prNumber, webhookHeadSha = null) {
  ...
  if (!currentHeadSha) throw new CustomRuleEvidenceError("Fresh PR metadata did not include a head SHA", { prNumber });
  ...
  const { data: verifiedPr } = await octokit.request(
    "GET /repos/{owner}/{repo}/pulls/{pull_number}",
    { owner, repo, pull_number: prNumber }
  );
  const verifiedHeadSha = verifiedPr?.head?.sha || null;
  const verifiedFiles = Number(verifiedPr?.changed_files);
  if (verifiedHeadSha !== currentHeadSha || verifiedFiles !== expectedFiles) {
    throw new CustomRuleEvidenceError("PR changed during evidence collection", {
      prNumber,
      expectedHeadSha: currentHeadSha,
      currentHeadSha: verifiedHeadSha,
      expectedFiles,
...
export async function evaluateAndExecuteCustomRules(eventName, payload, installation, principalId = null) {
  ...
  if (target.prBacked) {
    ...
    let fresh;
    try {
      fresh = await fetchFreshPRContext(freshOctokit, repo.owner.login, repo.name, target.number, webhookHeadSha);
    } catch (err) {
      logger.warn(
  ...
  logger.info(
    { repo: repoFullName, matchedRules: matched.map((m) => m.name), event: eventName }, "Custom rules matched"
  );
  ...
  for (const rule of matched) {
    ...
    const blocked = ruleResults.some((r) => r.outcome === "blocked");
    const failed = ruleResults.some((r) => r.outcome === "failed");
    const dryRunSuppressed = ruleResults.some((r) => r.outcome === "dry_run");
    ...
    const reason = decision === "error"
      ? `Custom rule '${rule.name}' matched — one or more actions failed`
      : decision === "blocked"
        ? `Custom rule '${rule.name}' matched — one or more actions were blocked`
        : decision === "dry_run"
...
async function executeAction({
  ...
  if (target.type === "pr" && action.action !== "skip") {
    let drift;
    try {
      drift = await checkPRHeadDrift(freshOctokit, owner, repo, target);
    } catch (err) {
      await recordBlocked(
        act.id,
        BLOCKED_REASONS.EVIDENCE_INCOMPLETE,
        { head_revalidation_error: err.message },
        context
      );
      return { outcome: "blocked", result: { blocked: true, reason: "PR head could not be revalidated" } };
    }
    if (drift) {
      await recordBlocked(act.id, BLOCKED_REASONS.TARGET_DRIFTED, drift, context);
      return { outcome: "blocked", result: { blocked: true, reason: "PR head drifted" } };
    }
  }
...
async function checkPRHeadDrift(octokit, owner, repo, target) {
  const { data: livePr } = await octokit.request(
    "GET /repos/{owner}/{repo}/pulls/{pull_number}",
    { owner, repo, pull_number: target.number }
  );
  const liveHeadSha = livePr?.head?.sha || null;
  if (liveHeadSha === target.headSha) return null;
  return {
    expected_head_sha: target.headSha,
    current_head_sha: liveHeadSha,
  };
}
