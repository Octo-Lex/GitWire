...
//
// Run: NODE_OPTIONS="--experimental-vm-modules" npx jest tests/stress/longevity.test.js --testTimeout=120000 --runInBand

import { get } from '../helpers.js';
import { sleep } from './stress-helpers.js';

const ROUNDS = 10;
const REQUESTS_PER_ROUND = 5; // Keep under rate limit

describe(`Longevity: ${ROUNDS} rounds × ${REQUESTS_PER_ROUND} requests`, () => {
  const roundTimes = [];

  test('Sustained read load stays stable', async () => {
    for (let round = 0; round < ROUNDS; round++) {
      const start = Date.now();
      const results = [];
      for (let i = 0; i < REQUESTS_PER_ROUND; i++) {
        results.push(await get('/api/repos'));
        await sleep(100);
      }
      ...
      if (!allOk) {
        ...
        expect(no500s).toBe(true);
        continue; // Skip this round's time for degradation check
      }
      await sleep(200);
    }
  ...
  test('Sustained mixed-endpoint load stays stable', async () => {
    const endpoints = [
  ...
  test('Health check remains stable throughout', async () => {
    const times = [];
    for (let i = 0; i < 20; i++) {
      const start = Date.now();
