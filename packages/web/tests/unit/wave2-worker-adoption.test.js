...
describe("Wave 2 — four-state adoption gate", () => {
  ...
  it("worker:webhook consumer IS wired (adoptWorker at webhookWorker.js:16)", () => {
    expect(isWired("worker:webhook")).toBe(true);
  });
  ...
  it("webhook:github and worker:webhook are distinct boundaries (no ambiguity)", () => {
    ...
    expect(workerRow.wired).toBe(true);
    // No ambiguous mapping between these two (different modules)
